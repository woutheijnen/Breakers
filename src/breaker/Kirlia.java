 ...
 */
public class Kirlia extends ObjetTouchable {
	/**
	...
	private int vie=40;
        ...
        public Kirlia(Game g, int x, int y, GestionMap actualistion){
            super(g,"kirlia",x,y);   
            gestion = actualistion;
            this.crier();
            gestion.setPokeBallActive(true);
            gestion.getPlayer().changeScore(97);
        }
	...
	public void
        move(long dt) {
            ...
            xp += dt;
            if(xp >= 30000){
                Gardevoir gardevoir = new Gardevoir(game(), this.getLeft(), this.getTop(), gestion);
                game().add(gardevoir);
